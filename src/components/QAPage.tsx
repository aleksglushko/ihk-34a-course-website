...
export default function QAPage() {
    ...
    return (
        <div id="faq-section" className="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8">
            <div className="max-w-4xl mx-auto">
                ...
                <div className="space-y-4">
                    {qaData.map((item) => (
                        <div key={item.id} className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                            <button onClick={() => toggleExpanded(item.id)} className="w-full px-6 py-4 text-left hover:bg-gray-50 transition-colors duration-200">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center space-x-3">
                                        <span className="text-xl" role="img" aria-label="icon">{item.emoji}</span>
                ...
                <div className="mt-8 text-center text-sm text-gray-500">
                    <p>
                        IHK Sachkundeprüfung §34a GewO | Bewachungsgewerbe | Türsteher Prüfung | Kaufhausdetektiv Ausbildung | Objektschutz Qualifikation
                    </p>
                </div>
            </div>
        </div>
